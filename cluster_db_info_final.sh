...
'' as INT_COUNT,
'' as VERSION,
$total_cpu as NO_Of_CPUs,
$total_ram_gb as TOTAL_RAM,
'' as CDB_SGA_HL,
'' as CDB_SGA_SL,
'' as CDB_PGA_HL,
'' as CDB_PGA_SL,
...
exit;
EOF
cat db_info.csv >> /tmp/cluster_info_$server.csv
}
...
$total_cpu as NO_Of_CPUs,
$total_ram_gb as TOTAL_RAM,
$cdb_sga_hl as CDB_SGA_HL,
$cdb_sga_sl as CDB_SGA_SL,
$cdb_pga_hl as CDB_PGA_HL,
$cdb_pga_sl as CDB_PGA_SL,
$cdb_size as CDB_SIZE_GB,
(select ROUND((value)/1024/1024/1024,2) "SGA_SIZE_GB" from v\$parameter where name = 'sga_max_size') as PDB_SGA_HL,
(select ROUND((value)/1024/1024/1024,2) "SGA_SIZE_GB" from v\$parameter where name = 'sga_target') as PDB_SGA_SL,
(select ROUND((value)/1024/1024/1024,2) "PGA_SIZE_GB" from v\$parameter where name = 'pga_aggregate_limit') as PDB_PGA_HL,
(select ROUND((value)/1024/1024/1024,2) "PGA_SIZE_GB" from v\$parameter where name = 'pga_aggregate_target') as PDB_PGA_SL,
(select ROUND((select sum(bytes)/1024/1024/1024 from v\$datafile where con_id=(select con_id from v\$pdbs where name='$pdb')) + (select  sum(bytes)/1024/1024/1024 from v\$tempfile where con_id=(select con_id from v\$pdbs where name='$pdb')),2) PDB_SIZE from dual) as PDB_SIZE_GB,
'$scan_name' as Scan_name,
'$db_port' as DB_PORT,
...
# Removing and re-creating cluster_info_$server.csv file.
rm -rf /tmp/cluster_info_$server.csv
touch /tmp/cluster_info_$server.csv
...
cdb_sga_hl_sql="select nvl(ROUND((value)/1024/1024/1024,2),0) from v\$parameter where name = 'sga_max_size'"
cdb_sga_sl_sql="select nvl(ROUND((value)/1024/1024/1024,2),0) from v\$parameter where name = 'sga_target'"
cdb_pga_hl_sql="select nvl(ROUND((value)/1024/1024/1024,2),0) from v\$parameter where name = 'pga_aggregate_limit'"
cdb_pga_sl_sql="select nvl(ROUND((value)/1024/1024/1024,2),0) from v\$parameter where name = 'pga_aggregate_target'"
db_size_sql="select nvl(ROUND(((select nvl(sum(bytes),0)/1024/1024/1024 from v\$datafile) +
(select nvl(sum(bytes),0)/1024/1024/1024 from v\$tempfile) +
(select nvl(sum(bytes),0)/1024/1024/1024 from v\$log) +
(select nvl(sum(BLOCK_SIZE*FILE_SIZE_BLKS),0)/1024/1024/1024 from v\$controlfile)),2),0) from dual"
cdb_size_sql="select nvl(ROUND(((select nvl(sum(bytes),0)/1024/1024/1024 from v\$datafile) - (select sum(d.CDB_SIZE_GB) from (select con_id, nvl(sum(bytes),0)/1024/1024/1024 CDB_SIZE_GB from v\$datafile group by con_id) d, v\$pdbs p where d.con_id=p.con_id) + (select nvl(sum(bytes),0)/1024/1024/1024 from v\$tempfile where con_id=1) + (select nvl(sum(bytes),0)/1024/1024/1024 from v\$log) + (select nvl(sum(BLOCK_SIZE*FILE_SIZE_BLKS),0)/1024/1024/1024 from v\$controlfile)),2),0) from dual"
...
services_sql="select distinct value from gv\$listener_network where type='SERVICE NAME'"
...
# Capturing required value from OS level.


total_cpu=$(lscpu | grep "CPU(s):" | grep -v NUMA | awk '{print $2}')
...
do
    ...
    then
		if [ -f $HOME/$db.env ]; then
			...
			db_port=$(srvctl config listener | grep TCP|cut -d ':' -f3)
			db_port=$(echo $db_port | sed ':a;N;$!ba;s/\n/ /g')
			scan_name=$(srvctl config scan | grep 'SCAN name' | awk '{ print $3 }' | cut -d '.' -f1 | sed 's/,//g')
			...
			if [[ "${open_mode[*]} " =~ "ORA-01507" ]]; then
				...
				if echo "${last_node_db_names[@]}" | grep -qv "$db_name"; then
					...
					then
						...
						db_version=$(get_db_sql_result "${db_version_sql}")
						cdb_sga_hl=$(get_db_sql_result "${cdb_sga_hl_sql}")
						cdb_sga_sl=$(get_db_sql_result "${cdb_sga_sl_sql}")
						cdb_pga_hl=$(get_db_sql_result "${cdb_pga_hl_sql}")
						cdb_pga_sl=$(get_db_sql_result "${cdb_pga_sl_sql}")
						db_size=$(get_db_sql_result "${db_size_sql}")
						cdb_size=$(get_db_sql_result "${cdb_size_sql}")
					...
					#Trim variable values for unwanted spaces
					cdb_sga_hl=`echo $cdb_sga_hl | sed 's/ *$//g'`
					cdb_sga_sl=`echo $cdb_sga_sl | sed 's/ *$//g'`
					cdb_pga_hl=`echo $cdb_pga_hl | sed 's/ *$//g'`
					cdb_pga_sl=`echo $cdb_pga_sl | sed 's/ *$//g'`
					db_size=`echo $db_size | sed 's/ *$//g'`
					cdb_size=`echo $cdb_size | sed 's/ *$//g'`
					instance_count=`echo $instance_count | sed 's/ *$//g'`
					
					echo "Capturing (container:$container db_version:$db_version cdb_sga_hl:$cdb_sga_hl cdb_sga_sl:$cdb_sga_sl cdb_pga_hl:$cdb_pga_hl cdb_pga_sl:$cdb_pga_sl db_size:$db_size cdb_size:$cdb_size instance_count:$instance_count)"
					...
					then
						...
						then
							...
							do
								#echo "DB: $db PDB: $pdb"
								services=$(get_pdb_sql_result "${services_sql}" "${pdb}")
								services=$(echo $services | sed -e 's/ /\n/g'| cut -d '.' -f1 | sed ':a;N;$!ba;s/\n/ /g')
								db_version=$(get_pdb_sql_result "${db19_version_sql}" "${pdb}")
								exec_sql_for_pdb "${pdb}"
							done
						else
							services=$(get_db_sql_result "${services_sql}")
							services=$(echo $services | sed -e 's/ /\n/g'| cut -d '.' -f1 | sed ':a;N;$!ba;s/\n/ /g')
					...
					echo "Database","Oracle ExaCC",""$cluster_name"",""$server"",""$db"","",""$env"",""$remarks"","","",""$total_cpu"",""$total_ram_gb"","","","","","","","","","","","","" > db_info_not_matched_db_name.csv
					cat db_info_not_matched_db_name.csv
					cat db_info_not_matched_db_name.csv >> /tmp/cluster_info_$server.csv
				fi
			fi
		else
			echo "Environment file $HOME/$db.env file doesn't exist"
			remarks="ERROR: $db.env file not exist"
			echo "Database","Oracle ExaCC",""$cluster_name"",""$server"",""$db"","",""$env"",""$remarks"","","",""$total_cpu"",""$total_ram_gb"","","","","","","","","","","","","" > db_info_no_env.csv
			cat db_info_no_env.csv
			cat db_info_no_env.csv >> /tmp/cluster_info_$server.csv
		fi
	else
...
if [[ $server != $execution_node ]]; then
	scp -q /tmp/cluster_info_$server.csv ${execution_node}:/tmp/
	rm -rf /tmp/cluster_info_$server.csv
fi
